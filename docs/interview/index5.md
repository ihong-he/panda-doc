...
// app.json
{
  "subPackages": [
    {
      "root": "packageA",        // 分包根目录路径
      "pages": ["page1", "page2"], // 分包包含的页面文件
      "name": "pack1"            // 分包别名（可选，用于独立分包预下载）
    }
  ]
}
```
...
|---------|---------|------|
| 📦 按需加载 | **组件懒加载** | 减少首屏渲染时间 |
| 📊 数据优化 | **数据聚合/采样** | 减少请求，避免大量数据点 |
| 🎨 渲染优化 | **requestAnimationFrame** | 控制渲染频率 |
| | **离屏渲染缓存** | 缓存静态图表 |
| | **Canvas绘制** | ECharts 默认支持 |
| 📈 性能监控 | **FPS监控** | 避免掉帧 |
| | **合理动画时长** | 避免卡顿 |
| ⚡ 其他 | **简化/关闭动画** | 降低渲染负担 |
| | **防抖/节流** | 控制交互事件 |
...
::: tip 💬 面试回答模板
"图表优化主要从数据、渲染、交互三方面入手：数据上做聚合和采样，减少数据量；渲染上用requestAnimationFrame控制频率，结合Canvas绘制和离屏缓存；交互上用防抖节流，并简化动画效果。同时监控FPS保证流畅度。"
:::
