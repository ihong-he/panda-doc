...
- **原型**：每个JavaScript对象都有一个内部属性 `[[Prototype]]`，通常称为原型（Prototype），指向它的原型对象。这个原型对象本身也是一个对象，因此也有自己的原型。通过这种方式，对象之间形成了一个链式结构，即原型链。

- **原型链**：当访问一个对象的属性时，如果该对象自身没有这个属性，JavaScript引擎会沿着原型链向上查找，直到找到目标属性或到达原型链的末端（`null`）。这种机制允许对象共享属性和方法，节省内存消耗并提高代码复用性。
