...
::: tip 📚 面试小贴士
本文档涵盖手写代码面试题，帮你快速掌握企业面试重点！重点理解原理，结合代码实例加深记忆。
:::
...
- **时间复杂度**：算法执行时间与输入规模的关系
  - O(n)：线性时间，与数据量成正比（如遍历一次数组）
  - O(n²)：平方时间，数据量翻倍时时间翻4倍（如双重循环）
- **空间复杂度**：算法所需内存与输入规模的关系
  - O(n)：需要与数据量成正比的额外空间（如创建新数组）
:::
...
- **外层循环 ( i )**: 控制要排多少轮，n 个数需要 n-1 轮
- **内层循环 ( j )**: 每轮比较相邻的两个元素，`n - 1 - i` 表示每轮减少比较次数，因为后面的元素已排好
- **交换逻辑**: `[arr[j], arr[j + 1]] = [arr[j + 1], arr[j]]` 使用 ES6 解构赋值，一行代码完成交换
- **swapped 标志**: 性能优化，如果某一轮都没有交换，说明已经排好序，直接退出
...
function quickSort(arr) {
  if (arr.length <= 1) {
    return arr;  // 数组为空或只有一个元素，无需排序，直接返回
  }

  // 【第2步：选择基准值】
  const pivot = arr[Math.floor(arr.length / 2)];  // 取中间元素作为基准
  // 为什么要选中间？可以避免最坏情况（已排序数组导致时间复杂度O(n²)）

  // 【第3步：初始化三个数组】
  const left = [];   // 存放比基准小的元素
  ...
  for (const num of arr) {
    if (num < pivot) {
      left.push(num);      // 比基准小，进左组
    } else if (num > pivot) {
      right.push(num);     // 比基准大，进右组
    } else {
      equal.push(num);     // 和基准相等，进相等组
    }
  }
...
```
第一轮：pivot=1（中间元素，索引为3）
  left:  []           比1小的
  equal: [1]          等于1的
  right: [5, 2, 9, 5, 6]  比1大的

递归处理 right [5, 2, 9, 5, 6]：
  pivot=5（中间元素，索引为2）
  left:  [2, 5]       比5小的
  equal: [5]          等于5的
  right: [9, 6]       比5大的
...
1. 首先，我们检查对象是否为Date、RegExp或其他特殊对象。如果是，则返回一个新的实例。
2. 创建一个新容器（数组或对象），用于保存复制后的对象。
3. 遍历对象，将每个属性复制到新容器中。
4. 返回新容器。
