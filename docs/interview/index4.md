...
- **Vue3**推荐组合式API（setup语法糖），相关逻辑放一起，代码更清爽

**🚀 性能提升**：
- **Vue3**编译优化更好，diff算法更智能，**静态节点不重新渲染**
...
- 通过`Object.defineProperty`把数据属性转成`getter/setter`
- 读取数据时收集依赖（哪个组件用了这数据）
- 修改数据时通知依赖的组件更新
...
> 💬 **要点**：子组件通过`defineEmits`自定义事件，`emit`触发并传数据，父组件使用`@`监听
