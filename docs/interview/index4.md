...
- **Vue2**：基于 `Object.defineProperty` 劫持对象属性，且无法检测对象新增属性、数组索引变化
- **Vue3**：基于 `Proxy` 代理整个对象，支持数组索引、新增属性等
...
- **编译优化**：Vue3 静态节点不参与 diff，打破层级的限制
...
**1. 父传子 - props**
- 父组件通过 `props` 绑定属性传值，子组件用 `defineProps` 接收

**2. 子传父 - emit**
- 子组件用 `defineEmits` 声明事件，通过 `emit` 触发，父组件监听

**3. 跨层级 - provide/inject**
- 祖先用 `provide` 提供数据，后代用 `inject` 注入使用

**4. 全局状态 - Pinia**
- 集中管理共享状态，适合大型项目
- 小项目可用 `reactive` 创建响应式对象导出共享

**5. 其他方式**
- `ref`：父组件获取子组件实例，直接调用方法
- `v-model`：双向绑定语法糖，本质是 props + emit
- `$attrs`：透传未被 props 接收的属性
